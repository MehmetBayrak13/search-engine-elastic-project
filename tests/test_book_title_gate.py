...
def _with_book_gate(**overrides):
    gate = dataclasses.replace(search_service.CONFIG.book_title_gate, **overrides)
    return dataclasses.replace(search_service.CONFIG, book_title_gate=gate)
...
def test_book_gate_clause_present_by_default():
    payload = search_service.build_search_query("kırmızı araba", apply_intent_reranking=False)
    gate = _book_gate_clause(payload)
    cfg = search_service.CONFIG.book_title_gate
...
def test_book_gate_query_text_flows_through_for_exact_title_query():
    ...
    payload = search_service.build_search_query(exact_title, apply_intent_reranking=False)
    gate = _book_gate_clause(payload)
...
def test_disabled_produces_no_book_gate_clause():
    search_service.CONFIG = _with_book_gate(enabled=False)
    payload = search_service.build_search_query("kırmızı araba", apply_intent_reranking=False)
    assert "must_not" not in payload["query"]["bool"]


def test_empty_categories_produces_no_book_gate_clause():
    search_service.CONFIG = _with_book_gate(categories=())
    payload = search_service.build_search_query("kırmızı araba", apply_intent_reranking=False)
    assert "must_not" not in payload["query"]["bool"]


def test_book_gate_categories_come_from_config():
    search_service.CONFIG = _with_book_gate(categories=("books", "buy a kindle", "audible audiobooks"))
    payload = search_service.build_search_query("kırmızı araba", apply_intent_reranking=False)
    gate = _book_gate_clause(payload)
