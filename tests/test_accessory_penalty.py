...
def _with_accessory_penalty(**overrides):
    ap = dataclasses.replace(search_service.CONFIG.accessory_penalty, **overrides)
    return dataclasses.replace(search_service.CONFIG, accessory_penalty=ap)
...
def test_penalizes_accessory_term_not_present_in_query():
    ...
    ratio = search_service.CONFIG.accessory_penalty.penalty
...
def test_does_not_penalize_term_the_user_explicitly_searched_for():
    ...
    payload = search_service.build_search_query("phone case", apply_intent_reranking=False)
    penalty_fns = _penalty_functions(payload)
    if penalty_fns:
        should = penalty_fns[0]["filter"]["bool"]["should"]
...
def test_disabled_leaves_query_unwrapped():
    search_service.CONFIG = _with_accessory_penalty(enabled=False)
    payload = search_service.build_search_query("wireless headphones", apply_intent_reranking=False)
...
def test_empty_terms_list_leaves_query_unwrapped():
    search_service.CONFIG = _with_accessory_penalty(terms=())
    payload = search_service.build_search_query("wireless headphones", apply_intent_reranking=False)
    assert "function_score" not in payload["query"]
...
def test_query_containing_every_configured_term_leaves_query_unwrapped():
    ...
    # eklenmemeli.
    search_service.CONFIG = _with_accessory_penalty(terms=("sticker",))
    payload = search_service.build_search_query("phone sticker", apply_intent_reranking=False)
    assert "function_score" not in payload["query"]
