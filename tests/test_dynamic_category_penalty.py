...
def _with_dynamic_intent(**overrides):
    di = dataclasses.replace(search_service.CONFIG.dynamic_intent, **overrides)
    return dataclasses.replace(search_service.CONFIG, dynamic_intent=di)
...
def test_wraps_query_with_must_not_terms_for_top_main_categories():
    ...
    assert penalty_fns[0]["weight"] == search_service.CONFIG.dynamic_intent.negative_category_penalty
...
def test_disabled_when_penalty_is_one_or_above():
    search_service.CONFIG = _with_dynamic_intent(negative_category_penalty=1.0)
    signals = _signals_with_discovery([
        {"value": "All Electronics", "field": "main_category", "doc_count": 500, "rank": 1, "source": "dynamic_category_discovery"},
    ])
    payload = search_service.build_search_query(
...
def test_disabled_when_dynamic_intent_disabled():
    search_service.CONFIG = _with_dynamic_intent(enabled=False)
    signals = _signals_with_discovery([
        {"value": "All Electronics", "field": "main_category", "doc_count": 500, "rank": 1, "source": "dynamic_category_discovery"},
    ])
    payload = search_service.build_search_query(
...
def test_no_extra_elasticsearch_request_reuses_already_computed_discovery():
    ...
    source = inspect.getsource(search_service._apply_dynamic_category_penalty)
