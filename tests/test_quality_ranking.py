...
def _with_quality_ranking(**overrides):
    """`app.CONFIG.quality_ranking`ı geçici olarak override eden bir AppConfig döner."""
    qr = dataclasses.replace(app.CONFIG.quality_ranking, **overrides)
    return dataclasses.replace(app.CONFIG, quality_ranking=qr)
...
def test_quality_ranking_enabled_wraps_query_in_function_score():
    app.CONFIG = _with_quality_ranking(enabled=True)
    payload = app.build_search_query("wireless headphones", apply_intent_reranking=False)
...
def test_quality_ranking_uses_configured_score_and_consistency_fields():
    app.CONFIG = _with_quality_ranking(enabled=True, score_field="dq_score", consistency_field="tc_consistency")
    payload = app.build_search_query("wireless headphones", apply_intent_reranking=False)
    functions = payload["query"]["function_score"]["functions"]
...
def test_low_consistency_penalty_function_uses_configured_threshold_and_weight():
    app.CONFIG = _with_quality_ranking(
...
def test_missing_quality_fields_never_break_query_shape_when_enabled():
    ...
    app.CONFIG = _with_quality_ranking(enabled=True, missing_value_behavior="neutral")
    payload = app.build_search_query("wireless headphones", apply_intent_reranking=False)
    functions = payload["query"]["function_score"]["functions"]
    for function in functions:
        if "field_value_factor" in function:
...
def test_missing_value_behavior_skip_uses_exists_filter():
    app.CONFIG = _with_quality_ranking(enabled=True, missing_value_behavior="skip", bypass_for_exact_asin=False)
    payload = app.build_search_query("wireless headphones", apply_intent_reranking=False)
    functions = payload["query"]["function_score"]["functions"]
...
def test_exact_asin_bypass_excludes_matching_document_from_all_functions():
    app.CONFIG = _with_quality_ranking(enabled=True, bypass_for_exact_asin=True)
    ...
    for function in functions:
        ...
        assert must_not[0]["term"][app.CONFIG.search_methods.exact_asin.field]["value"] == "B000123456"


def test_bypass_disabled_does_not_add_must_not_filter():
    app.CONFIG = _with_quality_ranking(enabled=True, bypass_for_exact_asin=False)
    payload = app.build_search_query("B000123456", enable_exact_asin=True, apply_intent_reranking=False)
    functions = payload["query"]["function_score"]["functions"]
    for function in functions:
...
def test_discovery_quality_filter_adds_must_not_range_when_enabled():
    app.CONFIG = _with_quality_ranking(discovery_filter_enabled=True, discovery_min_data_quality_score=0.4)
    payload = app.build_category_discovery_query("toilet paper")
    must_not = payload["query"]["bool"]["must_not"]
    assert must_not[0]["range"][app.CONFIG.quality_ranking.score_field]["lt"] == 0.4


def test_discovery_quality_filter_preserves_original_should_query():
    app.CONFIG = _with_quality_ranking(discovery_filter_enabled=True)
    payload = app.build_category_discovery_query("toilet paper")
